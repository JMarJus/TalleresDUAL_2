		...
		for (Person personGot: personService.getAllPeople()) {
			...
			for (Apartment apartmentGot: apartmentService.getAllApartments()) {
				...
				if (personGot.getApartmentId().equals(apartmentGot.getId())) {
					System.out.println(personService.getPersonByApartmentId(apartmentGot.getId()).getFirst().getName() + " " + personService.getPersonByApartmentId(apartmentGot.getId()).getFirst().getSurname());
